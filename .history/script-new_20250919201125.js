...
class StudyPlannerKanban {
    ...
    getDefaultColumns() {
        return [
            { id: 'backlog', name: 'Backlog', color: 'default', order: 1 },
            { id: 'active', name: 'Active', color: 'blue', order: 2 },
            { id: 'review', name: 'Review', color: 'orange', order: 3 },
            { id: 'finished', name: 'Finished', color: 'green', order: 4 }
        ];
    }
    ...
    exportToPDF() {
        ...
        sortedColumns.forEach(column => {
            ...
            htmlContent += `<div class="section-title">${column.name} (${columnCards.length} cards)</div>`;

            if (columnCards.length === 0) {
                htmlContent += `<div class="empty-section">No cards in this section</div>`;
            } else {
